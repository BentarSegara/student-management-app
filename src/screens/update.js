...
import { BookOpen, Columns, Info, Save, User } from "lucide-react-native";
...
const UpdatePage = ({ route }) => {
  ...
  const { currStudent } = route.params;
  const prodis = [
  ...
  const [student, setStudent] = useState({
    npm: currStudent.npm,
    nama: currStudent.nama,
    prodi: currStudent.program_studi,
  });
  ...
  const putData = async () => {
    setIsLoading(true);
    try {
      const response = await updateMahasiswa({
        id: currStudent.id,
      ...
      console.error("Gagal memperbarui data mahasiswa: ", err.message);
      return false;
    } finally {
      setIsLoading(false);
    }
